...
- **代码高亮**：Shiki 只在 playground（`src/components/shiki-code-block` 懒加载细粒度包 + JS 正则引擎，`main.ts` 同名覆写 `DCodeBlock` 注册，演示页零改动）；**库产物不内置**——DCodeBlock 默认插槽接受预高亮 HTML，宿主自行接入（写法见 demo 页「配合 Shiki 预高亮」）
