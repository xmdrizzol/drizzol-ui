...
- **46 处 `color-mix()` 收敛为三元组**：新增 `--dz-{primary,success,warning,danger,gray-7,gray-9,bg}-rgb`（随主题切换）与 `--dz-code-block-text-rgb`、`--dz-scrim-strong-rgb`（恒定）。**覆盖语义色时需与 `--dz-<名>` 成对同步**；暗色 `--dz-primary-hover-2` 同时改为静态值。
